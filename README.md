...
- **1.7.8** - Auto crop for a report photographed sideways. The page fills a landscape photo across its width, and the mixed-background recovery -- the only path that can describe it -- was scoped to portrait frames, so it refused; rotating 90 degrees already cropped it, which is what the hint says. That scope was an orientation proxy for a shape property: what actually keeps the recovery off a non-page is that its quad has to run along the frame's long axis, and testing the frame's aspect only approximates that when the frame is portrait. It now tests the shape directly, in whichever direction the frame runs. The portrait path is unchanged -- the old test was an early return, so every line after it was only ever reached on a portrait frame. Measured: removing the scope outright changes exactly one of the 66 corpus photos, a montage on a dark field whose only recovery is a 0.22 x 0.97 strip crossing its landscape frame, and that strip is what the shape test refuses. Testing the shape instead changes none of the 66, and pix71/pix72/pix73 are identical at all four rotations. An app-level "rotate and retry" alternative was measured and rejected: it would have cropped that montage at its import orientation, i.e. added a confident crop of a non-page. The 0.66 rescue-area cap, the frame-stretch refusals and every previously declined photo are unchanged. Auto all remains Auto crop + Auto clean in the already-selected tone mode; it does not choose Auto colour.

- **1.7.7** - Auto crop for a page that runs off the photo. A report photographed sideways has its long edge outside the frame, so the hull that would describe the page instead describes the photo and is correctly refused; the fallback edge geometry was then refused too, because the side it drew along the frame is not the page. That fallback's corners are still the right shape, so the page's boundary is now re-fitted from them: three sides land on the real edges and the fourth -- the one running off the frame -- keeps the frame. Two conditions are required, both inside the re-fit. A fitted line that runs across its own side is not that side's edge (the missing side otherwise fits the surface the page lies on, roughly ninety degrees out). And the re-fit may pull sides in but may not push the crop outward past the geometry the refusal already described, because a fit that grows the crop is following a boundary beyond the page. The two reported photos that were refused at all four rotations now crop when held upright. The 0.66 rescue-area cap, the mixed-background and frame-stretch refusals and every previously declined photo are unchanged: across 66 photos the crops are identical to 1.7.6, including the four declined cases that must stay declined. Auto all remains Auto crop + Auto clean in the already-selected tone mode; it does not choose Auto colour.

- **1.7.6** - Two taxi receipts from the same report. The first came back as a false "No clear page edge found" because a side whose paper is the darker of the two probes for part of its length was being asked which probe was brighter rather than whether the two differed at all; that side is now scored on the unsigned difference, so the receipt is cropped. The second was cropped with its left side sheared by 12% of the width: the left-side refinement could only accept a small nudge (area within 10% and the whole-quad score improving), which a shear repair can never satisfy, because un-shearing a left edge re-adds the wedge the shear had cut. It now also accepts a shear, judged on the side it repairs instead — the replacement line must be plumb, must be a strong boundary, and must beat the side it replaces — with the area allowed to grow by the wedge the move actually adds, computed rather than a raised cap. The three sides it does not edit are still required to stay put, so only the reported photo's crop changed. The 0.66 rescue-area cap, the mixed-background and frame-stretch refusals and every previously declined photo are unchanged: across 66 photos the crops of exactly the two reported receipts differ from 1.7.5 and the other 64 are identical. An independent edge opinion may refine the left edge but may not become the crop on its own — measured, it would have cut this receipt's last lines.

- **1.7.5** - Real-receipt fixes from four failing photos. Auto crop no longer refuses an obvious tall receipt (a false "No clear page edge found"), and it no longer clips the left side of a receipt whose interior seam scored higher than its paper edge. The wide-outward side refinement that 1.7.4 added is now scoped to the tall bounded wide receipt family it was written for: outside that family the earlier geometry is kept, so a receipt that was already cropped correctly is no longer trimmed back inward. The 0.66 rescue-area cap and the mixed-background and frame-stretch refusals are unchanged. Auto all remains Auto crop + Auto clean in the already-selected tone mode; it does not choose Auto colour.
